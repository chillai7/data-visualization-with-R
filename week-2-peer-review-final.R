...
table1 <- select(dat,'educ','edloan','employ','CC18_308a')
table1
...
#### Write your code below:
approval <- recode(table1$CC18_308a,'1'=1,'2'=1,'3'=0,'4'=0)
approval
table1 <- mutate(table1, approval)
table1 <- rename(table1, trump_aprrove_disapprove = approval)
table1
# Problem 3
...
#### Write your code below:
condition1<-  filter(dat,employ==1, marstat==1) %>% summarise(mean_importance_religion = mean(pew_religimp), median_importance_religion = median(pew_religimp))
condition1
