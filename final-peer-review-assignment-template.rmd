...
boxplot(x=dat$faminc_new,xlab='Family Income')
...
filter1 <- filter(dat, gender == 1, region == 1 | region == 2, marstat == 1, newsint == 1 )
str(filter1)
...
recode_for_325a <- recode(dat$CC18_325a, '1' = 1, '2' = 0)
recode_for_325d <- recode(dat$CC18_325d, '1' = 1, '2' = 0)
tax_scale <- recode_for_325a + recode_for_325d
dat1 <- mutate(dat, tax_scale)
print(dat1$tax_scale[1:20])
...
trump_approve_byregion <- dat %>% group_by(region) %>% summarise(Trump_Approve_mean = mean(CC18_308a))
trump_approve_byregion
...
filter2 <- filter(dat, investor == 2, faminc_new == 5 | faminc_new == 6 | faminc_new == 7 | faminc_new == 8 | faminc_new == 9 | faminc_new == 10 | faminc_new == 11 | faminc_new == 12 )
str(filter2)
filter2 %>% summarise(Mean_imp_religion = mean(filter2$pew_religimp), Median_imp_religion = median(filter2$pew_religimp), sd_imp_religion = sd(pew_religimp))
```
...
qplot(dat$pid7, xlab =  'seven point pary id', ylab = 'count')
