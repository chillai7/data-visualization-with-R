...
USA_oversea <- filter(my_world_map, region == 'Costa Rica' | 
                              region =='Panama' | 
                              region == 'Nicaragua')

my_cities <- maps::world.cities

USA_oversea_city <- filter(my_cities, country.etc=='Costa Rica'|
                                   country.etc=='Panama'|
                                   country.etc=='Nicaragua')

oversea_main_city <- filter(my_cities, country.etc=='Costa Rica'&
                                    pop>40000|
                                   country.etc=='Panama'&
                                    pop>40000|
                                   country.etc=='Nicaragua' &
...
oversea_main_city$measurement <- Measurement

ggplot(data = USA_oversea, mapping = aes(x = long, y = lat, group = group))+
        geom_polygon(fill = 'white', color = 'black')+
        geom_point(data = oversea_main_city, aes(x = long, y = lat, group = NULL, color = Measurement), size = 5)+
